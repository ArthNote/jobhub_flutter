...
class RecruiterChats extends StatefulWidget {
  const RecruiterChats({super.key});

  @override
  State<RecruiterChats> createState() => _RecruiterChatsState();
}

class _RecruiterChatsState extends State<RecruiterChats> {
...
class _BodyState extends State<Body> {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Expanded(
            child: StreamBuilder(
                stream: ChatStuff().getChats(uid, "Recruiter_id"),
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    return ListView(
                      padding: EdgeInsets.only(bottom: 8, top: 10),
                      children: snapshot.data!.docs.map((e) =>
                        ChatCard(e.get("Candidate_name"), e.get("Last_msg"), e.get("Time_sent"), e.get("RUnread_msg"), e.get("id"), e.get("Candidate_id"))
                      ).toList(),
  ...
  Widget ChatCard(String cname, String last_msg, String time_sent, int runread_msg, String id, String cid) {
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(
              ...
              child: Text(
                cname,
                style: TextStyle(
                    fontSize: 18, color: dark_blue, fontWeight: FontWeight.w500),
              ),
            ),
            Padding(
              padding: EdgeInsets.all(5),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            Text(time_sent,
                style: TextStyle(fontSize: 14, color: dark_blue)),
            SizedBox(
              height: 8,
            ),
            Container(
