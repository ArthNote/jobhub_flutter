...
class JobPage extends StatelessWidget {
  String job_id;
  JobPage({super.key, required this.job_id});
...
class _BodyState extends State<Body> {
  ...
  final AppStuff aStuff = AppStuff();
  ...
  void checkApplicationStatus() async {
    bool check = await aStuff.checkApp(uid, widget.job_id);
    setState(() {
      isApplied = check;
    });
  }
  ...
  void apply(String rid, String jid, String title, String company) async {
    setState(() {
      isApplying = true;
    });
    if (!isApplied) {
      String app_date = DateTime.now().toString();
      final newApp = Application("", uid, jid, rid,"Pending",app_date, title, company);
      bool r = await aStuff.applyToJob(newApp);
      setState(() {
        isApplying = false;
        isApplied = r;
      });
      if (r == true) {
        showToast(message: "Applied to Job successfully");
        setState(() {
        });
      } else {
        showToast(message: "Error Applying to Job");
        setState(() {
        });
      }
    }else{
      showToast(message: "Already Applied to This Job");
      setState(() {
          isApplying  = true;
      });
    }
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () => apply(job.rid, job.id, job.title, job.company), 
